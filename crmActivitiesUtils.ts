...
/**
 * Pipeline cards created when logging a call (interest / follow-up) mirror the hub sales-call record.
 * Drop those mirrors when the source sales call is already in the pool so Activities/Report don't duplicate rows.
 */
export function dedupePipelineMirrorCalls(leads: any[]): any[] {
    const list = Array.isArray(leads) ? leads : [];
    const hubIds = new Set<string>();
    for (const l of list) {
        const id = String(l?.id || '').trim();
        if (id && !id.startsWith('P')) hubIds.add(id);
    }
    return list.filter((l) => {
        const id = String(l?.id || '').trim();
        if (!id.startsWith('P')) return true;
        const sources = Array.isArray(l?.sourceCallIds) ? l.sourceCallIds : [];
        return !sources.some((sid: any) => hubIds.has(String(sid || '').trim()));
    });
}
...
/** Auto-created CRM call from a request deadline (offer / deposit / full payment). */
export function isRequestDeadlineAutoCall(lead: any): boolean {
    return String(lead?.autoGeneratedType || '').trim() === 'request_deadline';
}
...
function relatedLeadsForAccount(
    ...
    const focus = byId.get(fid);
    // Request-deadline auto calls: show every logged call for the same request (not just parent/follow-up thread).
    if (focus && isRequestDeadlineAutoCall(focus)) {
        const reqId = String(focus?.requestId || '').trim();
        if (reqId) {
            return all.filter(
                (l: any) =>
                    isRequestDeadlineAutoCall(l) && String(l?.requestId || '').trim() === reqId
            );
        }
    }

    // Manual calls: thread = focused call + parent chain + follow-ups only (not all account calls).
